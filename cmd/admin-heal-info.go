...
func printSetHealInfo(set madmin.SetStatus) string {
	...
	var speedObjectsPerSecond, speedBytesPerSecond uint64

	passedTime := healInfo.LastUpdate.Sub(healInfo.Started) / time.Second
	if passedTime > 0 {
		speedObjectsPerSecond = healInfo.ObjectsHealed / uint64(passedTime)
		speedBytesPerSecond = healInfo.BytesDone / uint64(passedTime)
	}

	fmt.Fprintf(&out, "       Start Time: %s\n", healInfo.Started)
	fmt.Fprintf(&out, "      Current Dir: %s/%s\n", healInfo.Bucket, healInfo.Object)
	fmt.Fprintf(&out, "       Scan Speed: %d obj/s, %s/s\n", speedObjectsPerSecond, humanize.IBytes(speedBytesPerSecond))
