...
func guessCopyURLType(sourceURLs []string, targetURL string, afterTimeRef bool, timeRef time.Time, isRecursive bool, keys map[string][]prefixSSEPair) (copyURLsType, *probe.Error) {
	if len(sourceURLs) == 1 { // 1 Source, 1 Target
		...
		if timeRef.IsZero() {
			...
			sourceContent, err = url2StatWithTimeRef(sourceURL, afterTimeRef, timeRef)
		}
		if err != nil {
...
func prepareCopyURLsTypeA(sourceURL string, targetURL string, afterTimeRef bool, timeRef time.Time, encKeyDB map[string][]prefixSSEPair) URLs {
...
func prepareCopyURLsTypeB(sourceURL string, targetURL string, afterTimeRef bool, timeRef time.Time, encKeyDB map[string][]prefixSSEPair) URLs {
	// Extract alias before fiddling with the clientURL.
	sourceAlias, _, _ := mustExpandAlias(sourceURL)
	// Find alias and expanded clientURL.
	targetAlias, targetURL, _ := mustExpandAlias(targetURL)
...
func prepareCopyURLsTypeC(sourceURL, targetURL string, afterTimeRef bool, timeRef time.Time, isRecursive bool, encKeyDB map[string][]prefixSSEPair) <-chan URLs {
	// Extract alias before fiddling with the clientURL.
	sourceAlias, _, _ := mustExpandAlias(sourceURL)
	// Find alias and expanded clientURL.
	targetAlias, targetURL, _ := mustExpandAlias(targetURL)
	...
	go func(sourceURL, targetURL string, copyURLsCh chan URLs) {
		...
		var srcCh <-chan *ClientContent

		if timeRef.IsZero() {
			srcCh = sourceClient.List(isRecursive, false, false, false, DirNone)
		} else {
			srcCh = listSourceWithTimeRef(sourceClient, afterTimeRef, timeRef, isRecursive, false, DirNone)
		}

		for sourceContent := range srcCh {
...
func prepareCopyURLsTypeD(sourceURLs []string, targetURL string, afterTimeRef bool, timeRef time.Time, isRecursive bool, encKeyDB map[string][]prefixSSEPair) <-chan URLs {
	copyURLsCh := make(chan URLs)
	go func(sourceURLs []string, targetURL string, copyURLsCh chan URLs) {
		defer close(copyURLsCh)
		for _, sourceURL := range sourceURLs {
			for cpURLs := range prepareCopyURLsTypeC(sourceURL, targetURL, afterTimeRef, timeRef, isRecursive, encKeyDB) {
				copyURLsCh <- cpURLs
			}
		}
...
func prepareCopyURLs(sourceURLs []string, targetURL string, isRecursive bool, encKeyDB map[string][]prefixSSEPair, olderThan, newerThan, timeRefStr string) chan URLs {
	copyURLsCh := make(chan URLs)
	...
	fmt.Println(afterTimeRef, timeRef)

	go func(sourceURLs []string, targetURL string, copyURLsCh chan URLs, encKeyDB map[string][]prefixSSEPair) {
		defer close(copyURLsCh)
		cpType, err := guessCopyURLType(sourceURLs, targetURL, afterTimeRef, timeRef, isRecursive, encKeyDB)
		fatalIf(err.Trace(), "Unable to guess the type of copy operation.")

		switch cpType {
		case copyURLsTypeA:
			copyURLsCh <- prepareCopyURLsTypeA(sourceURLs[0], targetURL, afterTimeRef, timeRef, encKeyDB)
		case copyURLsTypeB:
			copyURLsCh <- prepareCopyURLsTypeB(sourceURLs[0], targetURL, afterTimeRef, timeRef, encKeyDB)
		case copyURLsTypeC:
			for cURLs := range prepareCopyURLsTypeC(sourceURLs[0], targetURL, afterTimeRef, timeRef, isRecursive, encKeyDB) {
				copyURLsCh <- cURLs
			}
		case copyURLsTypeD:
			for cURLs := range prepareCopyURLsTypeD(sourceURLs, targetURL, afterTimeRef, timeRef, isRecursive, encKeyDB) {
				copyURLsCh <- cURLs
			}
		default:
